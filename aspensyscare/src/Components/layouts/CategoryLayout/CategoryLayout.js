...
const Drawer = ({ categoryid }) => {
  ...
  return (
    <div className='w-[70%] flex flex-wrap  content-start	justify-between p-5 bg-white h-auto'>
...
const CategoryLayout = () => {
  ...
  return (
    <div className='w-full flex flex-col flex-nowrap justify-center items-center h-[auto] '>
          ...
          category.map((val, i) => {
            return (
              <div onMouseEnter={() => setIsShown({ categoryid: val.id, show: true })}
                onMouseLeave={() => setIsShown({ categoryid: '', show: false })}>
