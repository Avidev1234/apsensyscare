    ...
    <Box sx={{ display: 'flex', height: trigger ? "80px" : "80px" }} >
      ...
      <AppBar component="nav" sx={{ background: '#0112FE', height: trigger ? "80px" : "80px", transition: '200ms ease-in' }} >
        <Toolbar style={{ minHeight: '80px' }}>
          {/* mobile view start   */}
          <Box sx={{ display: { sm: 'none' }, justifyContent: 'space-between' }} className='w-full flex flex-row gap-2 items-center' >
            <IconButton
              color="inherit"
            ...
            {opensearchbar ?
              <Search >
                <input type="text" placeholder="Type here" className="w-full text-[#696969] outline-none p-[10px] rounded-md"
            ...
            <Box className="!m-0" onClick={() => setOpensearchBar((prev) => !prev)}>
              {!opensearchbar ?
