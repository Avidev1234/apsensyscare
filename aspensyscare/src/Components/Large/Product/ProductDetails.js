...
import {
  addToWishlist,
  removeFromWishlist,
} from "../../../Store/Slices/getwishlistSlice";
...
const ProductDetails = (product) => {
  ...
  const curremtcategory =
    category !== undefined
      ? category?.filter((item) => item.id === product.products.category_id)
      : [];
  ...
  const itemIndex =
    details !== undefined
      ? details.filter((item) => item.product_id === product.products.id)
      : [];
  ...
  const handleCart = (product) => {
    ...
    const itemIndex =
      details !== undefined
        ? details.filter((item) => item.product_id === product.id)
        : [];

    let itemsize = "";
    // const Productvariants = [];
    // console.log("itemIndex",itemIndex.product_id);
    // itemIndex.map((item, idx) => {
    //     const index = size.filter((items) => items.id === item.size_id)
    //     const values = {
    //         "price": item.price,
    ...
    const ProductObj = Object.assign(
      { cartQuantity: val, price: currentSize.price },
      product
    );
    ...
    if (!isLoggedIn) {
      navigate(
        `/loginbefore-cart/${product.category_id}/${product.id}/${product.product_url}`,
        { state: { product } }
      );
    } else {
      navigate("/cart");
  ...
  return (
    <Detailscont>
      ...
      <div className="w-full ">
        <div className="w-full flex flex-row gap-x-2 justify-start item-start text-[#a7a6a6] mb-2">
          <div className="w-full text-sm m-0 flex flex-row gap-2">
            <a
              href="/"
              target="_blank"
              className="hover:text-[#997af6] hover:underline"
            >
              Home
            </a>
            <span>/</span>
            <a
              href={`/category/${curremtcategory[0].category_url}/c/${curremtcategory[0].id}?categoryId=${curremtcategory[0].id}`}
              target="_blank"
              rel="noreferrer"
              className="hover:text-[#997af6] hover:underline"
            >
              {curremtcategory[0].category_name}
            </a>
            <span>/</span>
        ...
        >
          Product category : {curremtcategory[0].category_name}
        </p>
      </div>
      <Rattingcont>
      ...
      <div className="flex flex-col">
        ...
        >
          ...
          {currentSize.price}.00
          <span className="text-[#bbbaba] mx-2 line-through text-[12px]">
            {Number(currentSize.mrp) > currentSize.price
              ? "₹" + currentSize.mrp + ".00"
              : ""}
          </span>
          <span className="text-green-600 text-[12px]">
            {currentSize.discount > 0 ? currentSize.discount + "% off" : ""}
          </span>
        </p>
        <p
          variant="subtitle2"
          ...
          ? Productvariants.map((items, id) => {
              return (
                <SizeButtom
                  ...
                  style={{
                    backgroundColor:
                      currentSize.size === items.size ? "green" : "#000",
                  }}
                  onClick={() => {
                    setCurrentSize({
                      size: items.size,
                      ...
                      mrp: items.mrp,
                      discount: Math.round(
                        ((items.mrp - items.price) / items.mrp) * 100
                      ),
                    });
                    setOpen(false);
                  }}
      ...
      <ButtomBox>
        <RemoveCircleRoundedIcon
          style={{
            fontSize: "18px",
            cursor: val >= 2 ? "pointer" : "not-allowed",
            color: val >= 2 ? "black" : "#d9d9d9",
          }}
      ...
      <ButtomBox>
        {!isLoggedIn && (
          <AddCart
            variant="contained"
            onClick={() => handleCart(product.products)}
          >
            Add Cart
          </AddCart>
        )}
        ...
        <QuickBuy
          variant="contained"
          onClick={() => {
            Wishlist(product.products, exsit);
          }}
        >
          Add to wishlist
        </QuickBuy>
