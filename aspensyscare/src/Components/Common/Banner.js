...
import PregressiveImage from '../Large/layouts/ImageLoader/PregressiveImage';
...
const Banner = ({ position, Header }) => {
  ...
  return (
    <div className="w-full mt-[2rem] flex flex-nowrap flex-col gap-4 max-h-[500px] h-auto lg:h-[500] ">
      {Header === true ? <div className='w-full font-bold pl-[6px]'>
      {!Banner.loading && banner !== undefined ? ( <h1 className='text-bold text-[18px] md:text-[24px] leading-[40px]'><strong>Welcome to the Apsensyscare Family</strong></h1>) : <Skeleton count={2} />}
        
      </div> : <h2 className='text-bold text-[18px] md:text-[24px] leading-[40px] pl-[8px]'><strong>Explore Essentials & More</strong></h2>}
      {bannerImages===null&&
